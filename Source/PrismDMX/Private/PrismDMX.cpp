...
bool UPrismDMX::FetchDMXBuffers(UDMXEntityFixturePatch* FixturePatch,
    ...
    // Ensure FixturePatch is valid before accessing properties
    ...
    else if (GEngine) { // Always warn when shifter signal is missing
        FString WarnStr =
    ...
    if (GEngine &&
        (!OutFixtureSignal.IsValid() ||
            OutFixtureSignal->ChannelData.Num() == 0)) { // Always warn when fixture signal is missing
        FString WarnStr =
            FString::Printf(TEXT("========== [ %s ] ==========\n[WARNING] Fixture "
...
void UPrismDMX::Modulate_Implementation(
    ...
    const bool bIsMatrixFixture = Mode->bFixtureMatrixEnabled;
    ...
    if (!bFoundIntensity && bIsMatrixFixture) {
    ...
    if (ShifterValue > ShifterThreshold) {
        ...
        if (bEnableDebug && GEngine && !bIsMatrixFixture) {
            ...
            // Use a distinct key to prevent overwrites
            GEngine->AddOnScreenDebugMessage(
                (uint64)GetTypeHash(FixturePatch->GetFName()) + 1, 0.0f, DebugColor,
                DebugStr);
        }
        ...
        if (bEnableDebug && GEngine && !bIsMatrixFixture) {
            ...
            GEngine->AddOnScreenDebugMessage(
                (uint64)GetTypeHash(FixturePatch->GetFName()) + 1, 0.0f, FColor::Orange,
                DebugStr);
        }
...
void UPrismDMX::ModulateMatrix_Implementation(
    ...
    // ==========================================
    // 3. CACHE MATRIX CONSTANTS & PARSE BASE VALUES
    // ==========================================
    ...
    // Parse baseline DMX console values from generic (non-matrix) functions
    float BaseIntensity = 0.0f, BaseCx = 0.0f, BaseCy = 0.0f;
    float BaseCCT = 0.0f, BaseTint = 0.0f, BaseColorCrossFade = 0.0f;

    if (FixtureSignal.IsValid()) {
        const TArray<uint8>& FixtureUniverseBuffer = FixtureSignal->ChannelData;
        for (const FDMXFixtureFunction& Func : Mode->Functions) {
            if (Func.Attribute == IntensityAttribute) bFoundIntensity = true;
            int32 AbsoluteAddr = StartAddress + Func.Channel - 1;
            if (!FixtureUniverseBuffer.IsValidIndex(AbsoluteAddr)) continue;
            ...
            if (Func.Attribute == IntensityAttribute) BaseIntensity = FinalVal;
            else if (Func.Attribute == CxAttribute) BaseCx = FinalVal;
            else if (Func.Attribute == CyAttribute) BaseCy = FinalVal;
            else if (Func.Attribute == CCTAttribute) BaseCCT = FinalVal;
            else if (Func.Attribute == TintAttribute) BaseTint = FinalVal;
            else if (Func.Attribute == ColorCrossFadeAttribute) BaseColorCrossFade = FinalVal;
        }
    }
    ...
    for (int32 CellIndex = 0; CellIndex < NumCells; CellIndex++) {
        ...
        // Initialise with bases, subsequently override with cell-specific
        float ConsoleIntensity = BaseIntensity, ConsoleCx = BaseCx, ConsoleCy = BaseCy;
        float ConsoleCCT = BaseCCT, ConsoleTint = BaseTint, ConsoleColorCrossFade = BaseColorCrossFade;
        ...
        if (ShifterValue > ShifterThreshold) {
            ...
            if (bEnableDebug && GEngine && CellIndex == 0) {
                ...
                GEngine->AddOnScreenDebugMessage(
                    (uint64)GetTypeHash(FixturePatch->GetFName()) + 2, 0.0f, DebugColor,
                    DebugStr);
            }
            ...
            if (bEnableDebug && GEngine && CellIndex == 0) {
                FString DebugStr = FString::Printf(
                    ...
                    *FixturePatch->GetName(), ConsoleIntensity, ConsoleCx, ConsoleCy);
                GEngine->AddOnScreenDebugMessage(
                    (uint64)GetTypeHash(FixturePatch->GetFName()) + 2, 0.0f, FColor::Orange,
                    DebugStr);
            }
